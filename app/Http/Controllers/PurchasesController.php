...
class PurchasesController extends Controller
{
    ...
    \DB::transaction(function () use ($request) {
        ...
        if ($request->hasFile('attachment')) {
            ...
            $path = $file->store('attachments/purchases', 'public'); // saved in storage/app/public/attachments/purchases
...
{
    if (PurchaseReturn::where('purchase_id', $id)->whereNull('deleted_at')->exists()) {
    ...
    DB::transaction(function () use ($request, $id) {
        ...
        $new_purchase_details = json_decode($request->details, true);

        if (!is_array($new_purchase_details)) {
            throw new \Exception('Invalid purchase details format');
        }

        $old_products_id = $old_purchase_details->pluck('id')->toArray();
        $new_products_id = collect($new_purchase_details)->pluck('id')->toArray();

        // Reverse old product stock
        foreach ($old_purchase_details as $detail) {
            $unit = Unit::find($detail->purchase_unit_id);
            $convertedQty = $unit ? ($unit->operator == '/' ? $detail->quantity / $unit->operator_value : $detail->quantity * $unit->operator_value) : $detail->quantity;
            ...
            if (!in_array($detail->id, $new_products_id)) {
                $detail->delete();
            }
        }

        // Update or insert new details and adjust stock
        foreach ($new_purchase_details as $prod_detail) {
            $unit = Unit::find($prod_detail['purchase_unit_id']);
            $convertedQty = $unit ? ($unit->operator == '/' ? $prod_detail['quantity'] / $unit->operator_value : $prod_detail['quantity'] * $unit->operator_value) : $prod_detail['quantity'];

            $product_warehouse = product_warehouse::whereNull('deleted_at')
                ->where('warehouse_id', $request->warehouse_id)
                ->where('product_id', $prod_detail['product_id'])
                ->when(!empty($prod_detail['product_variant_id']), fn($q) => $q->where('product_variant_id', $prod_detail['product_variant_id']))
                ->first();
            ...
            $detail_data = [
                'purchase_id' => $id,
                'date' => $request->date,
                'cost' => $prod_detail['Unit_cost'],
                'purchase_unit_id' => $prod_detail['purchase_unit_id'],
                ...
                'product_variant_id' => $prod_detail['product_variant_id'] ?? null,
            ...
            if (!in_array($prod_detail['id'], $old_products_id)) {
                ...
                PurchaseDetail::where('id', $prod_detail['id'])->update($detail_data);
            }
        }

        $due = $request->GrandTotal - $current_Purchase->paid_amount;
        $payment_statut = $due <= 0 ? 'paid' : ($due < $request->GrandTotal ? 'partial' : 'unpaid');
        ...
        $current_Purchase->update([
            ...
            'discount_percent_total' => $request->discount_percent_total ?? 0,
            'shipping' => $request->shipping ?? 0,
            'statut' => $request->statut ?? 'received',
        ...
        if ($request->hasFile('attachment')) {
            if ($current_Purchase->attachment) {
                Storage::delete($current_Purchase->attachment);
            }

            $filePath = $request->file('attachment')->store('attachments/purchases');
            $current_Purchase->update(['attachment' => $filePath]);
        }
    });
