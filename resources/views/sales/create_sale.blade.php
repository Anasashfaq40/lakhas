  ...
  <validation-provider name="Customer" rules="required" v-slot="{ valid, errors }">
    ...
    <v-select @input="Selected_Customer" v-model="sale.client_id" :reduce="label => label.value"
      ...
      :options="clients.map(clients => ({
        label: clients.username + (clients.address ? ' - ' + clients.address : ''), 
        value: clients.id
      }))"></v-select>
            ...
            <div class="row">
             ...
             {{-- Discount --}}
              ...
              <div class="col-lg-12 mt-5">
                <button @click="Submit_Sale" class="btn btn-primary" :disabled="paymentProcessing">
...
mounted() {
  if (this.warehouses.length > 0 && !this.sale.warehouse_id) {
