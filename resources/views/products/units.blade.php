        ...
        <div class="modal-body">
          <form @submit.prevent="editmode?Update_Unit():Create_Unit()" enctype="multipart/form-data">
            <div class="row">
              ...
              <div class="form-group col-md-12">
                <label for="ShortName">{{ __('translate.ShortName') }}</label>
              ...
              <div class="form-group col-md-12" v-show="show_operator">
                <label for="operator_value">{{ __('translate.Operation_Value') }}<span class="field_required">*</span></label>
                <input type="number" step="any" v-model="unit.operator_value" class="form-control" name="operator_value"
        ...
        buttons: [
          {
            ...
            buttons: [
              {extend: 'print', text: 'print', exportOptions: {columns: ':visible:Not(.not_show)', rows: ':visible'}},
              {extend: 'pdf', text: 'pdf', exportOptions: {columns: ':visible:Not(.not_show)', rows: ':visible'}},
              {extend: 'excel', text: 'excel', exportOptions: {columns: ':visible:Not(.not_show)', rows: ':visible'}},
              {extend: 'csv', text: 'csv', exportOptions: {columns: ':visible:Not(.not_show)', rows: ':visible'}},
            ]
          }
        ]
      });
  ...
  var app = new Vue({
    ...
    methods: {
      Selected_Base_Unit(value) {
        this.show_operator = value !== null;
      },

      Get_Data_Create() {
        axios.get("/products/units/create")
          .then(response => {
            this.units_base = response.data.units_base;
          })
          .catch(error => {
            toastr.error('Error loading unit data');
          });
      },

      Get_Data_Edit(id) {
        axios.get("/products/units/"+id+"/edit")
          .then(response => {
            this.unit = response.data.unit;
            this.units_base = response.data.units_base;
            this.show_operator = this.unit.base_unit !== null;
          })
          .catch(error => {
            toastr.error('Error loading unit data');
          });
      ...
      Update_Unit() {
        this.SubmitProcessing = true;
        axios.put("/products/units/" + this.unit.id, {
          name: this.unit.name,
          ShortName: this.unit.ShortName,
          base_unit: this.unit.base_unit,
          operator: this.unit.operator,
        ...
        swal({
          ...
          .then(response => {
            if (response.data.success) {
              ...
              toastr.error(response.data.message || '{{ __('translate.Unit_already_linked_with_sub_unit') }}');
            }
          })
          .catch(() => {
            toastr.error('{{ __('translate.There_was_something_wronge') }}');
